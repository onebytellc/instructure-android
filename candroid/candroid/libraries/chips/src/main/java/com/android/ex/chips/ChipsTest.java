...
public class ChipsTest extends AndroidTestCase {
    ...
    public void testReplaceChip() {
        ...
        view.replaceChip(mMockRecips[mMockRecips.length - 3], RecipientEntry
                .constructFakeEntry("replacement", "replacement@replacement.com", true));
    ...
    private void populateMocks(int size) {
        ...
        for (int i = 0; i < size; i++) {
            mMockEntries[i] = RecipientEntry.constructFakeEntry("user",
     ...
     * </p>
     */
    public void testCreateReplacementChipWithRecipient() {
        // Name in quotes + email address
        testCreateReplacementChipOriginalText(new RecipientEntry(1L, "Bob Boloblaw", "1234", "", "URL", 0, 0, true, null, null));
    }

    private void testCreateReplacementChipOriginalText(final RecipientEntry entry) {
        // No trailing space
        attemptCreateReplacementChipRecipientEntry(entry);
        // Trailing space
        attemptCreateReplacementChipRecipientEntry(entry);
    }

    private void attemptCreateReplacementChipRecipientEntry(final RecipientEntry entry) {
        final RecipientEditTextView view = new RecipientEditTextView(getContext(), null);

        view.setText(entry.getName());
        view.mPendingChips.add(entry);

        view.createReplacementChip(0, entry.getName().length(), view.getText(), entry);
        // The "original text" should be the email without the comma or space(s)
        assertEquals(entry.getName().replaceAll(",\\s*$", ""),
