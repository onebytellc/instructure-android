...
public class UserSubmissionsListAdapter extends BaseAdapter implements Filterable {
    ...
    private static void sortByName(List<Submission> submissions, final boolean showUngradedFirst){
            ...
            public int compare(Submission lhs, Submission rhs) {
                ...
                if(showUngradedFirst){
                    if((isGraded(lhs) && isGraded(rhs)) || (!isGraded(lhs) && !isGraded(rhs))) {
                        ...
                        return lhs.getUser().getSortableName().toLowerCase(Locale.getDefault()).compareTo(rhs.getUser().getSortableName().toLowerCase(Locale.getDefault()));
                    }
    ...
    private static void sortByGrade(List<Submission> submissions){
        Collections.sort(submissions, new Comparator<Submission>() {
            @Override
            public int compare(Submission lhs, Submission rhs) {
