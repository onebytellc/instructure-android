...
public class SubmissionCommentsFragment extends ParentFragment
    ...
    private void sendMessage(String message){
        if(message.trim().length() == 0) {
            ...
            SubmissionAPI.postSubmissionComment(getCanvasContext(), assignmentId, submission.getUser_id(), message, canvasCallbackMessage);
            hideKeyboard();
        }
    }
    ...
    public static Bundle createBundle(CanvasContext canvasContext, Submission submission, long assignmentId){
    ...
    public void onAttachButtonClicked() {
        Intent intent = KalturaMediaUploadPicker.createIntentForTeacherSubmissionComment(getContext(), submission.getAssignment(), submission.getUser().getId());
