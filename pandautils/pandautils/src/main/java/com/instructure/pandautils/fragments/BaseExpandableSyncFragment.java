...
public abstract class BaseExpandableSyncFragment<
    ...
    public void themeToolbar(@NonNull Toolbar toolbar) {
        final int textColor = ThemeUtils.getPrimaryText(getActivity());
        toolbar.setBackgroundColor(ThemeUtils.getPrimary(getActivity()));
        ToolbarColorizeHelper.colorizeToolbar(toolbar, textColor, getActivity());
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public void themeStatusBar() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            getActivity().getWindow().setStatusBarColor(ThemeUtils.darker(ThemeUtils.getPrimary(getActivity()), 0.85F));
        }
    }
