...
public abstract class BasePresenterActivity<PRESENTER extends Presenter<VIEW>, VIEW> extends PresenterActivity<PRESENTER, VIEW> {
    ...
    public void themeToolbar(@NonNull final Toolbar toolbar) {
        toolbar.post(new Runnable() {
            @Override
            public void run() {
                final int textColor = ThemeUtils.getPrimaryText(BasePresenterActivity.this);
                toolbar.setBackgroundColor(ThemeUtils.getPrimary(BasePresenterActivity.this));
                ToolbarColorizeHelper.colorizeToolbar(toolbar, textColor, BasePresenterActivity.this);
            }
        });
    }
    ...
    public void themeStatusBar() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            getWindow().setStatusBarColor(ThemeUtils.darker(ThemeUtils.getPrimary(BasePresenterActivity.this), 0.85F));
        }
    }
