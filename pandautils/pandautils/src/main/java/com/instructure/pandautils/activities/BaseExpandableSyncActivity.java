...
public abstract class BaseExpandableSyncActivity<
    ...
    public void themeToolbar(@NonNull final Toolbar toolbar) {
        toolbar.post(new Runnable() {
            @Override
            public void run() {
                final int textColor = ThemeUtils.getPrimaryText(BaseExpandableSyncActivity.this);
                toolbar.setBackgroundColor(ThemeUtils.getPrimary(BaseExpandableSyncActivity.this));
                ToolbarColorizeHelper.colorizeToolbar(toolbar, textColor, BaseExpandableSyncActivity.this);
            }
        });
    }
    ...
    public void themeStatusBar() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            getWindow().setStatusBarColor(ThemeUtils.darker(ThemeUtils.getPrimary(BaseExpandableSyncActivity.this), 0.85F));
        }
    }
