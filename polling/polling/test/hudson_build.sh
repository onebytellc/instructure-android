...
function compile_droid {
  ...
  if [ $compile_release == 1 ] ; then
    ...
    `chmod +x ./gradlew && ./gradlew clean assembleDebug --info > output_files/main_compile_output.log 2>&1`
  fi
...
function install_apks {
  /home/jenkins/Downloads/adt-bundle-linux-x86-20130219/sdk/platform-tools/adb install -r candroid/build/apk/candroid-debug-unaligned.apk
